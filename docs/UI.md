...
<div class="empty-state">
  ...
  <div class="empty-state-description">提交代码后，分析请求将显示在这里</div>
...
<FormField
  ...
  render={({ field, fieldState }) => (
    <FormItem>
      ...
      {fieldState.error && (
        <FormMessage className="text-sm text-danger">{fieldState.error.message}</FormMessage>
      )}
    </FormItem>
  )}
/>
...
_(注：具体业务术语应在各项目的 `DOMAIN.md` 中单独定义，但必须遵循上述动词规范)_
